	...
	for ( $j = $year_start; $j <= $year_end; $j++ )
	{
		    ...
		    for ( $i = 0; $i < $last; $i++ )
		    {
		        ...
		        if (@labels > 1)
		        {
		            ...
		            if ($holiday) {
		            	$return .= sprintf("\t'%04d-%02d-%02d': ",$year,$month,$day) . "[('$timestart', '$timeend')],\t# ($dow) " . join(", ", @labels) . "\n";
		            }
		        }
		    }
	}
...
unless (@ARGV == 5)
{
    die "Usage: perl timeperiod_holidays.pl YEAR_START YEAR_END PROFILE TIMESTART TIMEEND\n";
}
 
my $timeperiod = "timeperiods.update({'holidays': {\n\t'alias': 'holidays for Date::Calendar-Profile $ARGV[2] from $ARGV[0] to $ARGV[1]',\n";
